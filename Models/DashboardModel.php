<?php 
	class DashboardModel extends Mysql
	{
		...
		public function cantLeadsDji(){
			$sql = "SELECT COUNT(*) as total FROM tm_usuario";
			$request = $this->select($sql);
			$total = $request['total']; 
			return $total;
		}
		public function cantLeadsCons(){
			$sql = "SELECT COUNT(*) as total FROM tm_construccion";
			$request = $this->select($sql);
			$total = $request['total']; 
			return $total;
		}
		public function cantLeadsAgricola(){
			$sql = "SELECT COUNT(*) as total FROM tm_agricola";
			$request = $this->select($sql);
			$total = $request['total']; 
			return $total;
		}
		...
		public function lastOrders(){
			...
			$sql = "SELECT usu_id, usu_nom as nombre, est, usu_city FROM tm_usuario ORDER BY usu_id DESC LIMIT 10";
		...
		public function selectCiudadesMes(int $anio, int $mes){
			$sql = "SELECT usu_city FROM tm_usuario WHERE MONTH(fech_crea) = $mes AND YEAR(fech_crea) = $anvio";
		}
